...
public static class AuthHelper
{
  ...
  private static ClientSecrets? LoadClientSecrets(string path)
  {
    ...
    try
    {
      ...
      {
        Console.Error.WriteLine($"Fehler: Client Secrets konnten nicht geladen werden oder sind unvollständig in '{path}'. Prüfe die Datei.");
        return null;
      }
  ...
  public static async Task<UserCredential?> AuthenticateBasedOnEnvironmentAsync(
        ...
        string? dataStorePath = "YouTube.Auth.Store")
    {
        ...
        {
            Console.WriteLine("Keine Umgebungsvariable GOOGLE_REFRESH_TOKEN gefunden. Verwende lokale Authentifizierung (manueller Flow oder FileDataStore).");
        ...
        try
        {
            credential = await authenticator.AuthorizeAsync(cancellationToken);
        }
        catch (Exception ex)
        {
            // Fängt unerwartete Fehler während des AuthorizeAsync-Aufrufs ab
             Console.Error.WriteLine($"Ein schwerwiegender Fehler ist während der Authentifizierung aufgetreten: {ex.Message}");
             Console.Error.WriteLine("Stelle sicher, dass die Netzwerkverbindung besteht und die Client Secrets korrekt sind.");
             // Optional: Stack Trace für detailliertes Debugging ausgeben
             // Console.Error.WriteLine(ex.StackTrace);
             credential = null; // Sicherstellen, dass bei Fehlern null zurückgegeben wird
        }

        return credential; // Gibt das Ergebnis (Credential oder null) zurück
    }
}